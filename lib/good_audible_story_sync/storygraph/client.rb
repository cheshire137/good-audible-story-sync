  ...
  module Storygraph
    class Client
      ...
      def get_read_books
        ...
        first_page = load_page(-> { @agent.get(url) })

        filter_header_prefix = "Filter list "
        filter_header_el = first_page.search(".filter-menu *").detect do |el|
        ...
        books = get_read_books_on_each_page(page: first_page)
      ...
      sig { params(url: T.nilable(String), page: T.nilable(Mechanize::Page)).returns(T::Array[Book]) }
      def get_read_books_on_each_page(url: nil, page: nil)
        if url
          puts "#{Util::INFO_EMOJI} GET #{url}"
          page = load_page(-> { @agent.get(url) })
        elsif page.nil?
          raise "Either a URL or a page must be provided"
        end
        ...
        if next_page_link
          ...
          books += get_read_books_on_each_page(url: BASE_URL + next_page_link["href"])
        end
